...
function createAgencyIcon(type) {
  const colors = {
    police: '#4361ee',
    fire: '#ff3b5c',
    hospital: '#00d68f',
...
export default function ControlRoom() {
  ...
  const [previewResource, setPreviewResource] = useState(null); // { lat, lng, name, type }
  ...
  const [voiceAgentAlerts, setVoiceAgentAlerts] = useState([]);

  const [poiCategory, setPoiCategory] = useState('fire_station');
  ...
  const getPoiCenter = () => {
    if (selected?.location?.coordinates?.length === 2) {
  ...
  const fetchPlaces = async () => {
    ...
    setPoiLoading(true);
    try {
      const { lat, lng } = getPoiCenter();
      const { data } = await API.get('/places/nearby', {
        params: { lat, lng, radius: poiRadius, type: poiCategory },
      });
      setPois(Array.isArray(data?.results) ? data.results : []);
    } catch (e) {
      setPois([]);
      setPoiError(e?.response?.data?.message || 'Failed to load places');
    } finally {
      setPoiLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    const cleanups = [
      ...
      onEvent('voiceAgent:call', (payload) => {
        setVoiceAgentAlerts(prev => [{ ...payload, id: Date.now() }, ...prev]);
        toast(`📞 AI Agent Call — +91 ${payload.mobile}`, {
          duration: 8000,
          style: { background: 'rgba(220,38,38,0.2)', border: '1px solid #dc2626', color: '#fff' }
        });
      }),
    ];
  ...
  const handleSelect = async (incident) => {
    ...
    // If POI panel is already showing a category, refresh using the selected incident location.
  };
    ...
    <div className="app-layout">
      ...
      <div className="main-content">
        <div style={{ padding: '1rem', height: 'calc(100vh - 64px)' }}>
          ...
          {/* AI Voice Agent Call Alerts */}
          {voiceAgentAlerts.length > 0 && (
            <div style={{ marginBottom: '0.75rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {voiceAgentAlerts.slice(0, 3).map((alert) => (
                <div key={alert.id} style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: '0.78rem', fontWeight: 800, color: '#dc2626', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '0.25rem' }}>
                      🔴 AI Voice Agent Call Request
                    </div>
                  ...
                  <button
                    ...
                    onClick={() => setVoiceAgentAlerts(prev => prev.filter(a => a.id !== alert.id))}
                    style={{ flexShrink: 0 }}
                  >
                    <X size={14} />
                  </button>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              ...
              <div className="map-container" style={{ flex: 1, position: 'relative' }}>
                ...
                >
                  ...
                  {/* Preview Resource Marker & Route */}
                  {previewResource && (
                    <>
                      <Marker 
                        position={[previewResource.lat, previewResource.lng]} 
                        icon={createAgencyIcon(previewResource.type)}
                      >
                        <Popup>
                          <div>
                            <strong>{previewResource.name}</strong><br />
                            Type: {previewResource.type}<br />
                            (Preview Route)
                          </div>
                        </Popup>
                      </Marker>
                      {selected && (
                        <Polyline 
                          positions={[
                            [previewResource.lat, previewResource.lng],
                  ...
                  {/* Dispatched Fire Station Marker & Route */}
                  {selected?.assignedFireStation?.lat && selected?.assignedFireStation?.lng && (
                    <>
                      <Marker 
                        position={[selected.assignedFireStation.lat, selected.assignedFireStation.lng]} 
                        icon={createAgencyIcon('fire')}
                      >
                        <Popup>
                          <div>
                            <strong>{selected.assignedFireStation.name}</strong><br />
                            Dispatched Fire Station
                          </div>
                        </Popup>
                      </Marker>
                      <Polyline 
                  ...
                  {/* Dispatched Hospital Marker & Route */}
                  {selected?.assignedHospital?.lat && selected?.assignedHospital?.lng && (
                    <>
                      <Marker 
                        position={[selected.assignedHospital.lat, selected.assignedHospital.lng]} 
                        icon={createAgencyIcon('hospital')}
                      >
                        <Popup>
                          <div>
                            <strong>{selected.assignedHospital.name}</strong><br />
                            Dispatched Hospital
                          </div>
                        </Popup>
                      </Marker>
                  ...
                  {/* Dispatched Ambulance Marker & Route */}
                  {selected?.assignedAmbulance?.lat && selected?.assignedAmbulance?.lng && (
                    <>
                      <Marker 
                        position={[selected.assignedAmbulance.lat, selected.assignedAmbulance.lng]} 
                        icon={createAgencyIcon('ambulance')}
                      >
                        <Popup>
                          <div>
                            <strong>{selected.assignedAmbulance.name}</strong><br />
              ...
              {selected && (
                <div className="card" style={{ flexShrink: 0 }}>
                  ...
                  <RecommendationPanel 
                    incident={selected} 
                    onDispatch={(updatedIncident) => {
                      setSelected(updatedIncident);
                      // Update the incidents list with the new data
                      setIncidents(prev => prev.map(i => i._id === updatedIncident._id ? updatedIncident : i));
                    }}
                    onResourceClick={(lat, lng, name, type) => {
                      setPreviewResource({ lat, lng, name, type });
                    }}
                  />
                </div>
              )}
